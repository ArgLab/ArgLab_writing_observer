...
async def raw_ajax(runtime, target_url, lms_name, base_url=None, **kwargs):
    ...
    auth_info = {
        ...
        constants.SCHOOLOGY: request.get(constants.SCHOOLOGY_AUTH_SIGNING)
    }
    
    client_params = {}
    if auth_type == 'OAuth1':
        signing_info = auth_info[lms_name]
        api_key = signing_info['api_key']
        secret = signing_info['secret']
        
        # Manually sign the request with OAuth1
        oauth = requests_oauthlib.OAuth1(api_key, secret, "", "")
        req = requests.Request('GET', url, auth=oauth)
        prepped = req.prepare()  # This prepares the request and adds OAuth1 headers
        
        # Get the headers generated by OAuth1 signing
        signed_headers = dict(prepped.headers)
        # Convert all keys and values to strings (ensure all headers are strings)
        client_params['headers'] = {str(k): str(v) for k, v in signed_headers.items()}
    elif auth_type == 'OAuth2':
        client_params['headers'] = auth_info[lms_name]
    ...
    async with aiohttp.ClientSession(loop=request.app.loop) as client:
        try:
            async with client.get(url, **client_params) as resp:
...
async def raw_google_ajax(runtime, target_url, **kwargs):
    kwargs.setdefault('auth_type', 'OAuth2')
...
async def raw_canvas_ajax(runtime, target_url, **kwargs):
    ...
    kwargs.setdefault('auth_type', 'OAuth1')
...
async def raw_schoology_ajax(runtime, target_url, **kwargs):
    ...
    kwargs.setdefault('retry', True)
    kwargs.setdefault('auth_type', 'OAuth1')
    return await raw_ajax(runtime, target_url, constants.SCHOOLOGY, base_url, **kwargs)
